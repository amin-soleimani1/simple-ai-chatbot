...
export default function MessageBubble({ message }) {
  ...
  return <article className={`message-entry message-entry--${isUser ? "user" : "assistant"} flex ${isUser ? "justify-start" : "justify-end -ml-2 sm:-ml-3"}`}>{isPriceTable ? <div className={bubbleClass}><p className="mb-2 font-medium">{message.content}</p><PriceTable presentation={message.presentation} /></div> : <p className={`${bubbleClass} whitespace-pre-wrap`}>{message.content}</p>}</article>;
}
